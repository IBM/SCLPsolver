...
from .SCLP_subproblem7 import SCLP_subproblem
from .pivot_storage import pivot_storage


#'#@profile
def SCLP_pivot(Kset_0, Jset_N, solution, col_info, KK, JJ, NN, totalK, totalJ,
        ...
        if i2 == 1:
            if pp21[0] > 0:
                if DD[1:, 0][pbaseDD == pp21[0]][0] > 0:
                    ...
                    return solution, STEPCOUNT, ITERATION, pivot_problem
    if i1 == 1 and i2 == 1:
        solution.update_from_basis(col_info, piv1, AAN1, AAN2, JJ, KK, totalJ, totalK, pbaseDD, dbaseDD, DD)
        return solution, STEPCOUNT, ITERATION, pivot_problem
    else:
        sub_solution, STEPCOUNT, ITERATION, pivot_problem =\
         SCLP_subproblem(pbaseDD, dbaseDD, DD, v1, v2, Kset_0, Jset_N, AAN1, AAN2,
                            totalK, totalJ, DEPTH+1, STEPCOUNT, ITERATION, settings, tolerance)
        if pivot_problem['result'] == 0:
            solution.update_from_subproblem(col_info, sub_solution.pivots, AAN1, AAN2, JJ, KK, totalJ, totalK)
    return solution, STEPCOUNT, ITERATION, pivot_problem
