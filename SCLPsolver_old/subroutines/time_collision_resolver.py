...
def resolve_and_classify(delta, rz, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tol_coeff0, tolerance, shrinking_ind = None):
    problem = {'result': 0}
    tol2 = 10 * tolerance
    tol1 = tol2
    ...
    while tol_coeff <= 0.01/tolerance:
        ...
        if N1 == -1 and N2 == NN:
            ...
            return None, problem
        col_info = classify_time_collision(delta, rz, tol_coeff, N1, N2, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau,
                                pivots, tolerance)
        if col_info is None:
            had_resolution = True
        else:
...
def reclassify(col_info, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance):
    tol_coeff = col_info.tol_coeff * 10
    resolved = False
    while tol_coeff <= 0.01/tolerance:
        new_col_info, problem = resolve_and_classify(col_info.delta, col_info.rz, NN, klist, jlist, dx, dq, x, del_x, q,
                                                     del_q, tau, dtau, pivots, tol_coeff, tolerance)
        ...
        if new_col_info != col_info:
            ...
            break
    # if not resolved:
    #     result = ztau_resolver2(col_info, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance)
    #     if result is not None and result !=col_info:
    #         return result, True
    return col_info, resolved


def ztau_resolver(col_info, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance):
    if np.any(col_info.ztau_ind == col_info.N1):
        result = classify_time_collision(col_info.delta, col_info.rz, col_info.tol_coeff, col_info.N1-1, col_info.N2, NN,
                                         klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance)
        if result is None:
    ...
    if np.any(col_info.ztau_ind == col_info.N2):
        result = classify_time_collision(col_info.delta, col_info.rz, col_info.tol_coeff, col_info.N1, col_info.N2+1, NN,
                                         klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance)
        if result is None:
                ...
                if result is not None:
                    ...
                    if np.any(col_info.ztau_ind == col_info.N1):
                        result = classify_time_collision(col_info.delta, col_info.rz, col_info.tol_coeff,
                                                         col_info.N1 - 1,
                                                         col_info.N2 + 2, NN,
                                                         klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots,
                                                         tolerance)
                        if result is None:
                            if np.any(col_info.ztau_ind == col_info.N1 - 1):
                                result = classify_time_collision(col_info.delta, col_info.rz, col_info.tol_coeff,
                                                                 col_info.N1 - 2,
                                                                 col_info.N2 + 2, NN,
                                                                 klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau,
                                                                 pivots,
                                                                 tolerance)
                                if result is not None:
                                    return result
...
def ztau_resolver2(col_info, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance):
    import itertools
    ...
    for el in res:
        col = classify_time_collision(col_info.delta, col_info.rz, col_info.tol_coeff, el[0], el[1], NN,
                                klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance)
        if col is not None:
            return col
    return None
...
def classify_time_collision(delta, rz, tol_coeff, N1, N2, NN, klist, jlist, dx, dq, x, del_x, q, del_q, tau, dtau, pivots, tolerance):
    if N1 == -1 or N2 == NN:
        return collision_info('Case i__', delta, N1, N2, [], [], None, tol_coeff)
    else:
        vlist = pivots.get_out_difference(N1, N2)
        if len(vlist) > 2:
            ...
            case = 'Case ii_'
            order_ratio, correct = calc_order_ratio(vlist[0], vlist[1], N1, N2, klist, jlist, dx, dq, x, del_x, q,
                                                    del_q, tau, dtau, delta / 2)
