...
from subroutines.extract_rates import extract_rates
...
from subroutines.SCLP_solver4 import SCLP_solver
...
#
    ...
    base_sequence = {'dq': [dq], 'dx': [dx], 'bases': [{'prim_name': pn, 'dual_name': dn, 'A': A}], 'places': [0]}

    # Solve the problem, by a sequence of parametric steps
    #prim_name, dual_name, x_0, q_N, T, pivots, base_sequence = ...


    prim_name, dual_name, x_0, q_N, T, pivots,\
    base_sequence, STEPCOUNT = SCLP_solver(np.vstack(x_0), np.zeros((len(x_0),1)), np.vstack(q_N), np.zeros((len(q_N),1)), 0, 1, np.vstack(pn), np.vstack(dn), TT, 'toplevel',
                                           [], [], [], base_sequence, K_DIM + L_DIM, J_DIM + I_DIM, 0, 0, dict(), settings, tolerance)
    ...
    u, p = calc_controls(base_sequence, pivots, J_DIM + I_DIM, K_DIM + L_DIM)
    u = u.toarray()
    p = p.toarray()
    tau, dtau = calc_equations(np.arange(1, K_DIM+L_DIM+1), np.arange(1,J_DIM+I_DIM+1), pivots, x_0, np.zeros(len(x_0)), q_N, np.zeros(len(q_N)), T, 0, dx, dq)
    ...
    return t, x, q, u, p, pivots, obj, err, NN, STEPCOUNT