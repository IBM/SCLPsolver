...
def simplex_procedures(dct, ps, ds, tolerance = 0):
    ...
    if ptest.size > 0 and dtest.size == 0:
        ...
        while dtest.size > 0:
            j = dtest[0]
            mat = np.divide(dct.simplex_dict[1:, j + 1], dct.simplex_dict[1:, 0], out=np.zeros_like(dct.simplex_dict[1:, j + 1]), where=np.logical_and(dct.simplex_dict[1:, 0] != 0, ps != 1))
            i = np.argmax(mat)
            #i = np.argmax(mat * (ps != 1))
            if mat[i] <= 0:
                dct.simplex_dict[0, 0] = np.inf
        ...
        while mu > 0:
            if mu1 > mu2:
                ...
                mat = np.divide(dct2.simplex_dict[1:-1, j+1], div, out=np.zeros_like(dct2.simplex_dict[1:-1, j+1], -1), where= np.logical_and(div !=0, ps != 1))
                i = np.argmax(mat)
                if mat[i] <= 0:
    ...
    if ptest.size > 0 and dtest.size == 0:
        ...
        while mu > 0:
            if mu1 > mu2:
                ...
                mat = np.divide(LP_form2.simplex_dict[1:-1, j+1], div, out=np.zeros_like(LP_form2.simplex_dict[1:-1, j+1], -1), where= div !=0)
                i = np.argmax(mat)
                if mat[i] <= 0:
                    B[0, 0] = np.inf
                    err['result'] = 2
                ...
                mat = np.divide(-LP_form2.simplex_dict[i + 1, 1:-1], div, out=np.zeros_like(LP_form2.simplex_dict[i + 1, 1:-1], -1), where= div !=0)
                j = np.argmax(mat)
                if mat[j] <= 0:
                    B[0, 0] = - np.inf
                    err['result'] = 1
