...
#            result = 5  zero interval does not expand  data = non-expanding interval
    ...
    if zflag:
        ...
        ind2 = len(np.intersect1d(np.arange(zmin, zmax + 1, dtype=int), last_col_int, assume_unique=True)) == 0
            ...
            while tol_coeff >= 0.001 and zflag:
                ...
                if zflag:
                    ...
                    if last_case == 'Case ii_':
                        print('zero length interval shrinks:', ztau_ind, len(ztau_ind) / (zmax - zmin + 1))
                        if len(np.intersect1d(np.arange(zmin, zmax + 1, dtype=int), last_col_int, assume_unique=True)) != 0 and\
                                len(ztau_ind) / (zmax - zmin + 1) >= 0.65:
                            break
                else:
                    break
