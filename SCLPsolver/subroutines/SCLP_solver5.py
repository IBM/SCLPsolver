...
#function[ prim_name, dual_name, x_0, q_N, T, pivots, MatrixAA ] = ...
#'#@profile
def SCLP_solver(solution, x_0, del_x_0, q_N, del_q_N, T, del_T, ThetaBar, cases, B1, B2, klist, jlist, totalK, totalJ, DEPTH, STEPCOUNT, ITERATION, settings, tolerance):
    ...
    while cases != 'complete' and cases != 'solved__':
        ...
        # if check_complete and test1 == 0 and test2 == 0:
        #     cases, Delta, N1, N2, v1, v2, problem = get_complete(dx.shape[1])
        # else:
        cases, Delta, N1, N2, v1, v2, problem = classification(tau, dtau, klist, jlist, dx, dq, x, del_x, q, del_q,
                                                                   solution, B1, B2, sdx, sdq, tolerance, 1)
        if problem['result'] == 1 or problem['result'] == 2 or problem['result'] == 3:
            ...
            if problem['result'] == 2 and problem['timeProblem']['result'] == 2 and tol_coeff > 10000 and\
                ...
                while problem['result'] == 2 and tol_coeff > 0.0001:
                    ...
                    # else:
                    cases, Delta, N1, N2, v1, v2, problem = classification(tau, dtau, klist, jlist, dx, dq, x, del_x, q, del_q,
                                                                       solution.pivots, B1, B2, sdx, sdq, tolerance, tol_coeff)
            ...
            while tol_coeff <= 10000 and problem['result'] == 4:
                ...
                cases, Delta, N1, N2, v1, v2, problem = classification(tau, dtau, klist, jlist, dx, dq, x, del_x, q,
                                                                           del_q, solution.pivots, B1, B2, sdx, sdq, tolerance,
                                                                           tol_coeff)
            if problem['result'] == 0:
                tol_coeff = 1
                print('ok!')
        ...
        if problem['result'] > 0 and DEPTH == 0:
            if lastCollision['cases'] == 'Case ii_':
                ...
                solution.update_rewind(lastCollision['N1'], lastCollision['N2'], lastCollision['Nnew'],
                                       lastCollision['old_pn'], lastCollision['old_dn'],
                ...
                # else:
                cases, Delta, N1, N2, v1, v2, problem = classification(tau, dtau, klist, jlist, dx, dq, x, del_x, q,
                                                                           del_q, solution.pivots, B1, B2, sdx, sdq,
                                                                           tolerance, tol_coeff)
                if problem['result'] == 4:
                    while tol_coeff >= 0.001 and problem['result'] == 4:
                        ...
                        cases, Delta, N1, N2, v1, v2, problem = classification(tau, dtau, klist, jlist, dx, dq, x,
                                                                                   del_x, q, del_q, solution.pivots, B1,
                                                                                   B2, sdx, sdq, tolerance, tol_coeff)
                        if problem['result'] == 0:
                            tol_coeff = 1
                            print('ok!')
                        else:
...
def store_collision_info(N1, N2, lastCollision, solution):
    ...
    lastCollision['old_pn'] = solution.prim_name[:, cor_N1: N2].copy()
    lastCollision['old_dn'] = solution.dual_name[:, cor_N1: N2].copy()

def get_complete(NN):
    return 'complete', 0, -1, NN, [], [], {'result': 0}
