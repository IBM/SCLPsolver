...
def calc_init_basis(formulation, x_0, q_N, tolerance = 0):
    ...
    psx = ismember(np.arange(0,K), Kset).astype(int32)
    psu = -ismember(np.arange(J, J + I), Jset).astype(int32)
    ps = np.hstack((psx, psu))

    dn = np.hstack((-np.arange(1,J+1), np.arange(K + 1, K + L + 1)))
    dsq = ismember(np.arange(0,J), Jset).astype(int32)
    dsp = -ismember(np.arange(K, K + L), Kset).astype(int32)
