...
#           result = 2  multiple states hit zero    data = TODO
    ...
    JJ = len(jlist)
    #TODO: paralellize
    w_x = get_where(del_x, True, sdx, KK)
    rz_x, bb_x, kk_x, nn_x = calc_rz_bb(x, del_x, w_x)
    ###
    w_q = get_where(del_q, False, sdq, JJ)
    rz_q, bb_q, kk_q, nn_q = calc_rz_bb(q, del_q, w_q)
    #end
    if bb_x > bb_q:
        if bb_x == 0:
            print(kk_x, nn_x)
    ...
    #zstates = np.less(np.fabs(test2, where=w_x), tolerance, where = w_x, out=np.full_like(test2, False))
    sz_x = np.sum(zstates)
    ###
    ...
    #zstates = np.less(np.fabs(test2, where=w_q), tolerance, where = w_q, out=np.full_like(test2, False))
    sz_q = np.sum(zstates)
    #end
    if sz_x + sz_q > 1:
...
def get_where(del_state, is_primal, sd, vdim):
    if is_primal:
