...
def generate_MCQN_routing_data(seed, K, I, J, nz = 0.4, sum_rate=0.8, gdist=np.random.rand, gdist_params=(), h_0 = 0,
    ...
    P_K+= np.diag(np.full(K-1,0.1),-1)
    P_K /= np.outer(np.ones(K)+ coeff * np.random.rand(K), sum(P_K))
    G_K = np.eye(K) - P_K
    ...
    if gdist is np.random.rand:
        ...
        P_J = gdist(*gdist_params, (K,J-K))
    P_J -= (1 - nz) * np.ones((K, J-K))
    P_J[P_J < 0] = 0
    ...
    div = sum(P_J)
    np.add(div, np.ones(J-K), where = div == 0, out = div)
    coeff = (1 / sum_rate - 1) * 2
    P_J /= np.outer(np.ones(K) + coeff * np.random.rand(K), div)
    G_J = - P_J
    ...
    rows = np.random.choice(K, J-K, True)
    G_J[rows, cols] = 1
