...
results, ftrials, files, raw_tau = run_experiment_series('MCQN_routing', 10, 200, 20, 500, {'J':400,'alpha_rate':  1000},starting_seed =1000 )
pu = path_utils(os.path.expanduser('~/Box/SCLP comparison/data'))
DATADIRd = pu.get_CPLEX_data_path()
cplex_results = run_cplex_experiments(DATADIRd, relative_to_project('doe/cplex_integration/mod_files/main10defalt.mod'), files)
results = combine_results(results, cplex_results, '10defalt')
...
write_results_to_csv(results, res_file, raw_tau=raw_tau)